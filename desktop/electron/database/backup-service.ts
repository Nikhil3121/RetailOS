 ...
 */
/**
 * Is this typed confirmation good enough to replace the live database?
 *
 * THE RULE: the operator types the backup's own FILE NAME back.
 *
 * A confirmation dialog with a Yes button stops being a confirmation the third
 * time somebody sees it — it becomes a step to click through. A filename is
 * not typed by accident, and typing it also proves the person is looking at
 * the row they think they are, which a Yes button does not.
 *
 * Whitespace is trimmed because a name copied from a list picks up a trailing
 * space, and refusing over that teaches people to distrust the check rather
 * than to read it. Case is NOT ignored: a backup file name is generated by
 * this application, so the operator is reading it off the screen in front of
 * them, and there is nothing to be lenient about.
 *
 * Exported so the rule is testable on its own. A guard that only exists inside
 * an IPC handler is a guard nobody can prove still works.
 */
export function restoreConfirmationMatches(file: string, typed: unknown): boolean {
  if (typeof typed !== 'string') return false;
  const expected = path.basename(file);
  // An empty expected name would make every confirmation pass, including an
  // empty one. That cannot happen with a real path, and if it somehow did,
