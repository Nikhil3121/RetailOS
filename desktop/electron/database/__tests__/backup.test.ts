...
describe('the restore confirmation gate', () => {
  // A confirmation dialog with a Yes button stops being a confirmation the
  // third time somebody sees it. Typing the file name is what makes replacing
  // a live database a deliberate act rather than a step to click through.

  it('accepts the exact file name', () => {
    expect(restoreConfirmationMatches('/backups/retailos-2026-09-01.db',
      'retailos-2026-09-01.db')).toBe(true);
  });

  it('accepts a name with whitespace around it', () => {
    // Copied from a list, it picks up a trailing space. Refusing over that
    // teaches people to distrust the check rather than to read it.
    expect(restoreConfirmationMatches('/backups/retailos-2026-09-01.db',
      '  retailos-2026-09-01.db  ')).toBe(true);
  });

  it('refuses a different backup', () => {
    // THE CASE THAT MATTERS: right shape, wrong day. This is the mistake that
    // silently discards a week of bills.
    expect(restoreConfirmationMatches('/backups/retailos-2026-09-01.db',
      'retailos-2026-08-01.db')).toBe(false);
  });

  it('refuses an empty confirmation', () => {
    expect(restoreConfirmationMatches('/backups/x.db', '')).toBe(false);
    expect(restoreConfirmationMatches('/backups/x.db', '   ')).toBe(false);
  });

  it('refuses anything that is not a string', () => {
    // A renderer that sends undefined must not be treated as agreement.
    expect(restoreConfirmationMatches('/backups/x.db', undefined)).toBe(false);
    expect(restoreConfirmationMatches('/backups/x.db', null)).toBe(false);
    expect(restoreConfirmationMatches('/backups/x.db', true)).toBe(false);
  });

  it('is not fooled by the full path', () => {
    // The screen shows the file NAME. Accepting the path as well would mean
    // two right answers to a question with one.
    expect(restoreConfirmationMatches('/backups/x.db', '/backups/x.db')).toBe(false);
  });

  it('is case sensitive', () => {
    // The name is generated by this application and read off the screen in
    // front of the operator. There is nothing to be lenient about.
    expect(restoreConfirmationMatches('/backups/RetailOS.db', 'retailos.db')).toBe(false);
  });

  it('fails closed when there is no name to match', () => {
    expect(restoreConfirmationMatches('', '')).toBe(false);
  });

  it('handles a Windows path', () => {
    // Escaped backslashes: a single one here would be read as \b and \r,
    // which is a string with no path separators in it at all.
    expect(restoreConfirmationMatches('C:\\backups\\retailos.db', 'retailos.db')).toBe(true);
  });
});
